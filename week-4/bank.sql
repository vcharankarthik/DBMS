...
insert into branch values('sbi_chamraj','bangalore',50000),
('sbi_residency','bangalore',10000),
('sbi_shivaji','bombay',20000),
('sbi_parliment','delhi',10000),
('sbi_jantarmantur','delhi',20000);
...
select distinct(b.customer_name) from borrower b
join loan l on l.loan_no = b.loan_no
join bankacc a on l.branch_name = a.branch_name
join depositer d on a.acc_no = d.acc_no
where d.customer_name = b.customer_name;
select * from  branch;

select b.branch_name from branch b
where b.assets_in_lakhs > all(
select assets_in_lakhs from branch 
